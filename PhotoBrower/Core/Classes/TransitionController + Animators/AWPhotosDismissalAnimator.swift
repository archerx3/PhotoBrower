//
//  AWPhotosDismissalAnimator.swift
//  PhotoBrower
//
...
class AWPhotosDismissalAnimator: AWPhotosTransitionAnimator, UIViewControllerInteractiveTransitioning {
    ...
    weak fileprivate var overlayView: AWOverlayView?
    ...
    override func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if let from = from as? AWPhotosViewController {
            photosViewController = from
        } else {
            guard let childViewController = from.children.filter({ $0 is AWPhotosViewController }).first as? AWPhotosViewController else {
                assertionFailure(
                    """
                    Could not find AWPhotosViewController in container's children. \
                    This should never happen. If this does happen, the animator will complete \
                    the \"transition\" and return immediately.
                    """
                )
        ...
        let imageViewContainer = AWImageViewTransitionContainer(imageView: imageView)
        ...
        let scaleCompletion = { [weak self] (_ finished: Bool) in
            ...
            if self.canPerformContextualDismissal() {
                ...
                if let imageView = imageViewContainer.imageView as? FLAnimatedImageView,
                    ...
                    endingView.aw_syncFrames(with: imageView)
                }
