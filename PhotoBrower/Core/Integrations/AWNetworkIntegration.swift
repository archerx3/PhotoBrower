...
open class AWNetworkIntegration: NSObject, AWNetworkIntegrationProtocol, AWNetworkIntegrationURLSessionWrapperDelegate {
    
    fileprivate var urlSessionWrapper = AWNetworkIntegrationURLSessionWrapper()
    public weak var delegate: AWNetworkIntegrationDelegate?
    
//    fileprivate var dataTasks = NSMapTable<AWPhotoProtocol, URLSessionDataTask>(keyOptions: .strongMemory, valueOptions: .strongMemory)
    fileprivate var dataTasks = NSMapTable<AWPhotoProtocol, URLSessionDataTask>(keyOptions: .strongMemory, valueOptions: .strongMemory)
    fileprivate var photos = [Int: AWPhotoProtocol]()
    ...
    public func cancelLoad(for photo: AWPhotoProtocol) {
    ...
    // MARK: - AWNetworkIntegrationURLSessionWrapperDelegate
    fileprivate func urlSessionWrapper(_ urlSessionWrapper: AWNetworkIntegrationURLSessionWrapper,
...
// This wrapper abstracts the `URLSession` away from `AWNetworkIntegration` in order to prevent a retain cycle
// between the `URLSession` and its delegate
fileprivate class AWNetworkIntegrationURLSessionWrapper: NSObject, URLSessionDataDelegate, URLSessionTaskDelegate {
    
    weak var delegate: AWNetworkIntegrationURLSessionWrapperDelegate?
...
fileprivate protocol AWNetworkIntegrationURLSessionWrapperDelegate: NSObjectProtocol, AnyObject {
    
    func urlSessionWrapper(_ urlSessionWrapper: AWNetworkIntegrationURLSessionWrapper,
                           dataTask: URLSessionDataTask,
                           didUpdateProgress progress: CGFloat)
    func urlSessionWrapper(_ urlSessionWrapper: AWNetworkIntegrationURLSessionWrapper,
                           task: URLSessionTask,
